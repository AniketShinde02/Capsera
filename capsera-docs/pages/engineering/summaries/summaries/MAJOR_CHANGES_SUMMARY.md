...
## 🚀 **Features & Fixes (November 2025)**

### **One-Click Social Sharing**
- **New Feature**: Added share button to Caption Cards
- **Platforms Supported**: 
...
### **Advanced SEO System**
- **Crawler-Only Content**: Implemented hidden FAQ & "How it Works" sections for AI/LLM indexing (AEO)
- **Structured Data**: Added `SoftwareApplication` and `FAQPage` JSON-LD schemas
- **Metadata Optimization**: Updated titles and descriptions for "viral caption generator" keywords
- **Clean UI**: SEO content remains invisible to users to maintain design aesthetics

### **Brand & UI Identity Refresh**
- **New Logo**: Deployed modern `logo-v2.png` across the application
- **Typography**: Implemented "Space Grotesk" font for the "Capsera" brand name
...
- **Safety Filters**: Fixed silent failures when AI flagged content (e.g., "seduction")
- **User Feedback**: Added specific, friendly error messages for safety violations
- **Client-Side Validation**: Immediate feedback for potentially unsafe keywords

### **Files Modified**
- `src/components/caption-card.tsx` - Added sharing logic and dropdown menu
- `src/app/layout.tsx` - Updated metadata, fonts, and structured data
- `src/app/page.tsx` - Added (and hid) SEO content, optimized layout
- `src/components/hero.tsx` - Updated button styling
- `src/ai/flows/generate-caption.ts` - Improved safety error handling

## 📱 **Mobile UI & Marketing Enhancements (November 2025 - Part 2)**

### **Mobile Experience Overhaul**
- **Magic UI Mobile Menu**: Completely redesigned mobile navigation with glassmorphism, grid layout, and smooth animations.
- **Profile Responsiveness**: Optimized sidebar and layout for mobile devices, ensuring a seamless experience on small screens.
- **Hero Section Optimization**: Reduced height and spacing on mobile to bring content closer to the user.
- **Footer Redesign**: Reordered footer elements for mobile (Brand first, then Links) for better information hierarchy.

### **Contact System Improvements**
- **API Reliability**: Fixed `undefined` subject issue in contact form submissions by correctly mapping subject to category.
- **Enhanced Logging**: Improved server-side logging for better debugging of contact requests.

### **Viral Marketing Integration**
- **Smart Sharing**: Added promotional marketing text ("✨ Generated by Capsera...") to all shared captions.
- **Link Integration**: Embedded direct links to Capsera in shared content to drive organic traffic.

### **Performance & Infrastructure**
- **Edge Runtime Optimization**: Fixed Webpack cache name conflicts in `next.config.ts` to support Edge runtime without warnings.
- **Build Stability**: Resolved "unique cache name" warnings during development and build processes.

### **Files Modified**
- `src/components/server-header.tsx` - New mobile menu design
- `src/components/footer.tsx` - Mobile footer layout
- `src/components/hero.tsx` - Mobile spacing adjustments
- `src/app/contact/page.tsx` & `src/app/api/contact/route.ts` - Contact form fixes
- `src/components/caption-card.tsx` - Marketing text in sharing
- `next.config.ts` - Webpack cache fix
